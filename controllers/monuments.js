...
export async function getMonuments(req, res) {
    ...
    try {
        let monumentsOfType;
        if (types){
            monumentsOfType = (await MonumentTypesItem.findAll({where: {type: types.toString().split(",")}})).map(item=>item.monument_id);
            console.log(monumentsOfType);
        }
        const monuments = await Monument.findAll({where: {...(wilaya && {wilaya_name: wilaya}),
                ...(monumentsOfType&&{id: monumentsOfType}),title:{[Op.like]:`%${q}%`},
        },attributes:{exclude:["summary","rating","title"]}});
...
export async function getMonument(req, res) {
    ...
    try {
        ...
        monument.dataValues.images =  (await Image.findAll({where:{monumentId:id}})).map(image=>image.url);
...
export async function getMonumentReviews(req,res){
    ...
    try {
        ...
        const usersReviews = await Review.findAll({where:{monumentId:id}});
        const externalReviews = await ExternalReview.findAll({where:{monumentId:id}});
...
export async function createMonumentReview(req,res){
    ...
    try {
        ...
        const review = await Review.create({userId:req.user.id,monumentId:id,comment})
