...
export async function getContent360(req, res) {
    ...
    try {
        ...
        const page_limit = Math.ceil((await Content360.count())/Number(page_size));
...
export async function createContent360(req, res) {
    const { title,vr_link,latitude,longitude,location,image_url,wilaya } = req.query;
    try {
        if (!(title&vr_link&latitude&longitude&location&image_url&wilaya)){
        ...
        const content360 = await Content360.create({title,vr_link,latitude,longitude,location,image_url,wilaya_name:wilaya});
